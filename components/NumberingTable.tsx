...
export const NumberingTable: React.FC<NumberingTableProps> = ({ discrepancies, onViewParagraph }) => {
    return (
        ...
        <tbody className="divide-y divide-gray-200 dark:divide-midnight-light bg-white dark:bg-midnight">
            ...
            const paraNum = d.paragraphId ? (parseInt(d.paragraphId.replace('para-', '')) + 1) : null;
            return (
                <tr 
                ...
                >
                    ...
                    {paraNum !== null ? (
                        <button
                            onClick={() => onViewParagraph(d.paragraphId, d.documentId)}
                            className="hover:underline text-teal dark:text-lilac transition-colors"
                            title={`Go to Para ${paraNum}`}
                        >
                            {`Para ${paraNum}`}
                        </button>
                    ) : (
